	...
	if(par->type==2)
	  {
	    ...
	    //printf("\n par3 fit: %.2f, chisq/ndf: %.2f\n",f->GetParameter(3),f->GetChisquare()/f->GetNDF());
	    
	    g->Fill(f->GetParameter(3));
	  }
      }
 }
...
int main(int argc, char *argv[])
{
 ...
 par=(ShapePar*)malloc(sizeof(ShapePar));
  ...
  f=new TF1("wfit",myfunction,0,MAX_SAMPLE_LEN,5);
  g=new TH1D("TF","TF",S1K,0,S1K-1);
...
//here we write a ROOT file with all the histograms
  sprintf(title,"wfit_fit_fast_component.root");
  TFile f(title, "recreate");

  g->GetXaxis()->SetTitle("fast component rise [10 ns bins]");
