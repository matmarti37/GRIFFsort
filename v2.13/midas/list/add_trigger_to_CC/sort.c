...
int main(int argc, char *argv[])
{
  ...
  if(argc!=5)
    {
      printf("list_add_trigger_to_CC fragment_list map low_limit_ns high_limit_ns\n");
      exit(-1);
    }
  
 
  printf("Program assigns trigger number to CC detectors based on TIP trigger number on a list of fragments created from GRIF3/GRIF4 midas file.\n");
