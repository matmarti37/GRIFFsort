  ...
  if(fgets(line,132,inp)!=NULL)
    {
      if(fgets(line,132,inp)!=NULL)
				{
					numel=fscanf(inp,"%d %d %d %d %d",&pos,&col,&val1,&val2,&val3);
					while(numel!=EOF)
						{
							if(numel==4) //single csi in map
								{
									if(val1>0 && val1<NCSI)
										if(pos>0 && pos<NPOSTIGR)
											if(col>=0 && col<NCOL)
												{
													TIGRESS_cal_par->groupflag[pos][col][val1][0]=1;
													TIGRESS_cal_par->group_map[pos][col][val1][0]=val2;
													//printf("pos %d col %d csi %d group %d\n",pos,col,val1,val2);
												}
								}		
							else if(numel==5) //2 csi in map
								{
									if(val1>0 && val1<NCSI)
										if(val2>0 && val2<NCSI)
											if(pos>0 && pos<NPOSTIGR)
												if(col>=0 && col<NCOL)
													{
														TIGRESS_cal_par->groupflag[pos][col][val1][val2]=2;
														TIGRESS_cal_par->group_map[pos][col][val1][val2]=val3;
														//printf("pos %d col %d csi1 %d csi2 %d group %d\n",pos,col,val1,val2,val3);
													}
								}
							numel=fscanf(inp,"%d %d %d %d %d",&pos,&col,&val1,&val2,&val3);	
						}
											...
											if((raw_event->tg.det[pos].ge[col].h.THP&1)!=0)
												{
												  ...
												  //// START COMMENT HERE
													...
													/* t+=S16K; */
												  //// END COMMENT HERE
												  //t=((raw_event->tg.det[pos].ge[col].seg[0].timestamp_up&0xffff)<<32);
												  t=((raw_event->tg.det[pos].ge[col].seg[0].timestamp&0xfffc0000)<<4);
												  t+=raw_event->tg.det[pos].ge[col].seg[0].cfd;
												  t*=10;
												  t/=16;
		  ...
		  if(((raw_event->tg.det[pos].bgo[col].h.THP&(1<<sup))!=0))
		    {
		      ...
		      //t=((raw_event->tg.det[pos].bgo[col].sup[sup].timestamp_up&0xffff)<<32);
		      t=((raw_event->tg.det[pos].bgo[col].sup[sup].timestamp&0xfffc0000)<<4);
		      t+=raw_event->tg.det[pos].bgo[col].sup[sup].cfd;
		      t*=10;
		      t/=16;
