...
int analyze_data(raw_event *data)
{
  ...
  // Check total events vs GRIFFIN events
  if(cev->tg.h.FA>0)
...
int main(int argc, char *argv[])
{
  ...
  h = new TH2D("ECalABSuppbyCrystal","ECalABSuppbyCrystal",S32K,0,S32K,64,0,64);
  h->Reset();
  ...
  strcpy(rootFile,"ECalCrystal.root");
  TFile f(rootFile,"recreate");
  h->GetXaxis()->SetTitle("Energy [keV]");
  h->GetYaxis()->SetTitle("Crystal");
  h->SetOption("COLZ");
  h->Write();
  f.Close();

  double ratio;
  ratio=(double)tot_count/(double)grf_count;

  printf("TOTAL COUNTS:      %d\n",tot_count);
  printf("GRIFFIN COUNTS:    %d\n",grf_count);
  printf("RATIO (tot/grf):   %.2f\n",ratio);
  printf("RATIO*(3/16):      %.2f\n",ratio*3./16.);
}
