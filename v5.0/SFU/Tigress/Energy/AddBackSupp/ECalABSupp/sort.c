  	      ...
  	      if((cev->tg.h.AHP&(1<<(pos-1)))!=0)
  		{
			...
			//check the fold
  			if(cev->tg.det[pos].ge[col].h.FH>0)
			  {
			    ...
			    //printf("%f\n",cev->tg.det[pos].ge[col].suppress); //Matt's line
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=4)
    {
      printf("sfu_Tigress_ECalABSupp master_file_name supLow supHigh\n\n");
      printf("supLow and supHigh are from timing spectra. Run sfu_TigressBGO_TDiff\n");
      printf("NON-suppressed events will have supLow of 0\n");
      exit(-1);
    }
