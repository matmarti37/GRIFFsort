	...
	if(wpar.t10t90_flag==1)
	  ...
	  if(par->type==1)
		...
		h->Fill(i,waveform[i]);
	      ...
	      //	      printf("\n par4 fit: %.2f, chisq: %.2f\n",f->GetParameter(6),f->GetChisquare());
	      
	      gslow->Fill(f->GetParameter(6));
	      gfast->Fill(f->GetParameter(3));
...
int main(int argc, char *argv[])
{
 ...
 par=(ShapePar*)malloc(sizeof(ShapePar));
  ...
  gslow=new TH1D("TS/chisq","TS/chisq",S1K,0,S1K-1);
  gfast=new TH1D("TF/chisq","TF/chisq",S1K,0,S1K-1);
...
//here we write a ROOT file with all the histograms
  ...
  gslow->GetYaxis()->SetTitle("Counts");
  ...
  gfast->GetYaxis()->SetTitle("Counts");
