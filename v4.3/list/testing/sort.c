  ...
  if(curr->chan>=map->tig_min)
	...
	if(map->hpge_lt[curr->chan-map->tig_min].pos>0)//channel present on the CC list
	  if(map->hpge_lt[curr->chan-map->tig_min].pos<NPOSTIGR)//channel present on the CC list   
		...
		{
		  tsstore=curr->tsns;
		  chanstore=map->hpge_lt[curr->chan-map->tig_min].pos;
	      ...
	      if(diff<0)
		{
	      ...
	      energy=curr->ch.charge/E_divisor;
	      if(diff>0)
		if(diff<S4K)
		  if(energy>=Egate_low)
		    if(energy<=Egate_high)
		      if(map->hpge_lt[curr->chan-map->tig_min].pos==chanstore)
		      {
			hist[(int)diff]++;
			h->Fill(diff);
		      }
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=6)
    {
      printf("list_HPGe_CC_time_difference list_input_data map E_gate_low E_gate_high E_divisor\n");
      exit(-1);
    }
