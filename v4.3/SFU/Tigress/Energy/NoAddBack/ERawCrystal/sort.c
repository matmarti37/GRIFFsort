...
int main(int argc, char *argv[])
{
  ...
  if(argc!=3)
    {
      printf("sfu_Tigress_CC_ERaw sfu_input_data_file_name divider\n");
      exit(-1);
    }
  ...
  h=new TH2D("ERawbyCrystal","ERawbyCrystal",S32K,0,S32K,4*NPOSTIGR,0,4*NPOSTIGR);
  h->Reset();
  ...
  TFile f("ERawCrystal.root","recreate");
