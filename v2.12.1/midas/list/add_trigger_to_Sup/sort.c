...
int main(int argc, char *argv[])
{
  ...
  if(argc!=4)
    {
      printf("list_add_trigger_to_Sup fragment_list map window_ns\n");
      exit(-1);
    }
  
 
  printf("Program assigns trigger number to BGO suppressors based on HPGe central contact trigger number on a list of fragments created from GRIF3/GRIF4 midas file.\n");
