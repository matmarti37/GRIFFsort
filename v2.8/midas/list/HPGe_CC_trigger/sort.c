	 ...
	 if((curr.ch.timestamp|curr.ch.timestamp_up)!=0)//ignore bad events at the start
	   if(curr.chan>=map.tig_min)//check the range for TIGRESS channels
	     if(curr.chan<=map.tig_max)//check the range for TIGRESS channels
	       if(map.tig_lt[curr.chan-map.tig_min].seg==0)//channel present on the CC list
		 if(map.tig_lt[curr.chan-map.tig_min].pos>0)//channel present on the CC list
		   if(map.tig_lt[curr.chan-map.tig_min].pos<NPOSTIGR)//channel present on the CC list	       
		     {
