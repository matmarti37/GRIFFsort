...
void print_tccmap(gmap* map)
{
  ...
  for(i=0;i<TIGPOSLT;i++)
    if(map->tig_lt[i].pos>0)
      if(map->tig_lt[i].pos<NPOSTIGR)
	if(map->tig_lt[i].seg==0)
	printf("%5d             %5d %8s\n",i+map->tig_min,map->tig_lt[i].pos,col[map->tig_lt[i].col]);
   printf("================    ==========================================\n");
}
/*======================================================================*/
void print_tsegmap(gmap* map)
{
  ...
  for(i=0;i<TIGPOSLT;i++)
    if(map->tig_lt[i].pos>0)
      if(map->tig_lt[i].pos<NPOSTIGR)
	if(map->tig_lt[i].seg>0)
	  if(map->tig_lt[i].seg<NSEGTIGR)	  
	    printf("%5d             %5d %8s %8d\n",i+map->tig_min,map->tig_lt[i].pos,col[map->tig_lt[i].col],map->tig_lt[i].seg);
   printf("================    ==========================================\n");
}
/*======================================================================*/
			...
			if(ch>=0)
			  if(ch<TIGPOSLT)
			    if(pos>0)
			      if(pos<NPOSTIGR)
				{
				  map->tig_lt[ch].pos=pos;
				  map->tig_lt[ch].seg=0;
				  if(strcmp(col,"B")==0) map->tig_lt[ch].col=0;
				  if(strcmp(col,"G")==0) map->tig_lt[ch].col=1;
				  if(strcmp(col,"R")==0) map->tig_lt[ch].col=2;
				  if(strcmp(col,"W")==0) map->tig_lt[ch].col=3;		  
				}
		      }
		    fclose(tcc_inp);		    
		  }
