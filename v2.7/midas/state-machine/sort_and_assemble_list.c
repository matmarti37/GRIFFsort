  ...
  while(1)
    {
      i--;
      if(i==-1) break;
      //      printf("i %d tsns[i] %16lld tsns_s %16lld tsns[i+1]  %16lld\n",i,list[i].tsns,store.tsns,list[i+1].tsns);
      if(store.tsns>=list[i].tsns)
	if(store.tsns<=list[i+1].tsns)
	  break;
    }
  //  printf("i=%d    <===== ts add %16lld\n",i,list[dp->last-1].tsns);
  if(i==-1)//shift the whole list
    {
      //      printf("insert fragment at the start\n");
      memmove(&list[1],&list[0],(dp->last-1)*sizeof(node));
      memcpy(&list[0],&store,sizeof(node));
      return 1;
    }

  //shift the identified block
  //  printf("ordering fragments\n");
...
int analyze_fragment_for_assembly(Grif_event* ptr, short* waveform,data_pointers* dp, node* list,gmap* map)
{
  ...
  if(nd.chan>=map->csiarray_min)
	...
	if(map->csipos[nd.chan-map->csiarray_min]>0)
	  if(map->csipos[nd.chan-map->csiarray_min]<NCSI)
...
int print_list(int limit, int offset, node* list)
{
  ...
  for(i=0;i<limit;i++)
    printf("Frag: %9d Ts[ns] %16lld Ch %4d Ts_up %8u Ts %8u CFD %8u Trig %8d\n",i+offset,list[i].tsns,list[i].chan,(unsigned int)list[i].ch.timestamp_up,(unsigned int)list[i].ch.timestamp,(unsigned int)list[i].ch.cfd,list[i].trig);
